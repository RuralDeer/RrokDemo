...
public class SharedCookieStore implements IHtppCookieStore {
    ...
    public SharedCookieStore(Context context, String spName, String spCookieNamePrefix) {
        this.cookiePrefsName = TextUtils.isEmpty(spName) ? COOKIE_PREFS_NAME : spName;
        this.cookieNamePrefix = TextUtils.isEmpty(spCookieNamePrefix) ? COOKIE_NAME_PREFIX : spCookieNamePrefix;
        this.cookieNamePrefix = COOKIE_NAME_PREFIX;
        init(context);
    }

    private void init(Context context) {
        cookiePrefs = context.getSharedPreferences(cookiePrefsName, Context.MODE_PRIVATE);
        cookies = new HashMap<>();
        //将持久化的cookies缓存到内存中,数据结构为 Map<Url.host, Map<CookieToken, Cookie>>
        Map<String, ?> prefsMap = cookiePrefs.getAll();
        for (Map.Entry<String, ?> entry : prefsMap.entrySet()) {
            if ((entry.getValue()) != null && !entry.getKey().startsWith(cookieNamePrefix)) {
                //获取url对应的所有cookie的key,用","分割
                String[] cookieNames = TextUtils.split((String) entry.getValue(), ",");
                for (String name : cookieNames) {
                    //根据对应cookie的Key,从xml中获取cookie的真实值
                    String encodedCookie = cookiePrefs.getString(cookieNamePrefix + name, null);
    ...
    private void saveCookie(HttpUrl url, Cookie cookie, String cookieToken) {
        ...
        prefsWriter.putString(cookieNamePrefix + cookieToken, SerializableCookie.encodeCookie(url.host(), cookie));
        prefsWriter.apply();
    }
    ...
    public synchronized boolean removeCookie(HttpUrl url, Cookie cookie) {
        ...
        //文件移除
        SharedPreferences.Editor prefsWriter = cookiePrefs.edit();
        if (cookiePrefs.contains(cookieNamePrefix + cookieToken)) {
            prefsWriter.remove(cookieNamePrefix + cookieToken);
        }
        prefsWriter.putString(url.host(), TextUtils.join(",", cookies.get(url.host()).keySet()));
        prefsWriter.apply();
        return true;
